    ...
    <section className="mx-auto max-w-7xl px-4 py-16 md:px-6">
      <Surface>
        <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-sm uppercase tracking-[0.32em] text-primary">Города</p>
            <h2 className="mt-4 font-display text-4xl font-semibold">Лидерборд по городам</h2>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-muted-foreground">Пользователь сам указывает город в профиле. Топ игроков и городов считается по победам, поражениям, проценту побед и рейтингу.</p>
          </div>
        ...
        <div className="mt-8 grid gap-3 md:grid-cols-3">
          {players.map((player, index) => (
            <div key={player.id} className="rounded-3xl border border-border bg-muted/45 p-4">
              ...
              <p className="mt-1 text-sm text-primary">{player.rating} рейтинг · {player.win_rate}% побед</p>
            </div>
          ))}
        </div>
      </Surface>
