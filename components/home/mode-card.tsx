    ...
    <motion.div whileHover={{ y: -6 }} transition={{ type: "spring", stiffness: 260, damping: 22 }}>
      <Link href={href} className="group block h-full rounded-[2rem] border border-border bg-card/62 p-5 shadow-soft transition hover:border-primary/50 hover:bg-card">
        <div className="flex items-center justify-between gap-3">
          <span className="rounded-full bg-muted px-3 py-1 text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">{level}</span>
        ...
        <span className="mt-6 inline-flex items-center text-sm font-semibold text-primary">Открыть режим <ArrowRight className="ml-2 size-4 transition group-hover:translate-x-1" /></span>
      </Link>
    </motion.div>
  );
}